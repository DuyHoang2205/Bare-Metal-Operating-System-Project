...
void framebf_init(int x_offset, int y_offset) {
    ...
    mBuf[5] = 1280;              // Value(width)
    mBuf[6] = 720;               // Value(height)
    ...
    mBuf[13] = 8;
    mBuf[14] = 0;
    mBuf[15] = x_offset; // x offset
    mBuf[16] = y_offset; // y offset
    ...
    if (mbox_call(ADDR(mBuf), MBOX_CH_PROP) // mailbox call is successful ?
        ...
        uart_puts("\033[2J\033[H"); // Clear the screen before using it
        ...
        uart_puts("Press 'Space' to move to the next section"); // The terminal printed part should be put here, because it won't have any effect, but if you leave it in the main program, it will be very difficult, and more coding.
    } else {
